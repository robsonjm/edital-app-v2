    ...
    const initAuth = async () => {
      try {
        ...
        if (err.code === 'auth/admin-restricted-operation') {
          setError("⚠️ Login Anônimo Desativado: Vá no Firebase Console > Authentication > Sign-in method e ative o provedor 'Anonymous'.");
        } else {
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 selection:bg-blue-100">
      <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b h-16 sticky top-0 z-50 shadow-sm border-slate-100 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
          ...
          {user && <div className="w-10 h-10 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-black text-slate-500 border-2 border-white dark:border-slate-700 shadow-sm transition-transform hover:scale-110">{user.uid.charAt(0).toUpperCase()}</div>}
        </div>
      </header>
